...
export default function HomeScreen() {
  ...
  function renderWeatherStrip() {
    ...
    if (!weather || !weather.isReal) return (
      <View style={styles.weatherStrip}>
        <Text style={styles.weatherText}>🌤 Weather unavailable — enable location for updates</Text>
      </View>
    );
    return (
      <View style={styles.weatherStrip}>
        <View style={styles.weatherRow}>
          <Text style={styles.weatherMain}>{weather.emoji} {weather.temp}°C — {weather.description}</Text>
          <Text style={styles.weatherCity}>📍 {weather.cityName}</Text>
        </View>
        {weather.warning && <Text style={styles.weatherWarning}>⚠️ {weather.warning}</Text>}
            ...
            return (
              <View key={task.id} style={[styles.taskCard, task.completed && styles.taskCardCompleted]}>
                ...
                <View style={styles.progressBarBg}>
                  <View style={[styles.progressBarFill, { width: `${task.progress || 0}%`, backgroundColor: task.completed ? COLORS.success : COLORS.primary }]} />
                </View>
...
const styles = StyleSheet.create({
  ...
  weatherStrip: { backgroundColor: COLORS.primaryLight, borderRadius: 12, padding: 12, marginBottom: 16 },
  weatherRow: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: 4 },
  weatherMain: { fontSize: 14, fontWeight: "600", color: COLORS.primaryDark },
  weatherCity: { fontSize: 12, color: COLORS.primary, fontWeight: "500" },
  weatherWarning: { fontSize: 13, color: COLORS.warning, fontWeight: "500", marginBottom: 4 },
  weatherDetail: { fontSize: 11, color: COLORS.textSecondary },
  ...
  aiLabel: { fontSize: 11, fontWeight: "700", color: "rgba(255,255,255,0.7)", letterSpacing: 1, marginBottom: 6 },
  ...
  collabSubtitle: { fontSize: 13, fontWeight: "600", color: COLORS.textSecondary, marginBottom: 10 },
  ...
  joinMessage: { marginTop: 8, fontSize: 13, color: COLORS.textSecondary, textAlign: "center" },
  shareCodeBox: { marginTop: 12, backgroundColor: COLORS.primaryLight, borderRadius: 10, padding: 12, alignItems: "center" },
  shareCodeText: { fontSize: 18, fontWeight: "700", color: COLORS.primary, letterSpacing: 3 },
  shareCodeHint: { fontSize: 12, color: COLORS.textSecondary, marginTop: 4 },
  taskCard: { backgroundColor: COLORS.surface, borderRadius: 16, padding: 16, marginBottom: 12, shadowColor: "#000", shadowOpacity: 0.05, shadowRadius: 6, elevation: 2 },
  ...
  progressBtnText: { fontSize: 11, fontWeight: "600", color: COLORS.textSecondary },
